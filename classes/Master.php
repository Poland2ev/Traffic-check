...
Class Master extends DBConnection {
	...
	function save_offense(){
		...
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "Mã lỗi vi phạm đã tồn tại.";
			return json_encode($resp);
			exit;
		}
		if(empty($id)){
		...
		if($save){
			$resp['status'] = 'success';
			if(empty($id))
				$this->settings->set_flashdata('success',"Lỗi vi phạm mới đã được lưu.");
			else
				$this->settings->set_flashdata('success',"Lỗi vi phạm đã được cập nhật.");
		}else{
			$resp['status'] = 'failed';
	...
	function delete_offense(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"Lỗi vi phạm đã được xóa.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_driver(){
		...
		extract($_POST);
		$name = ucwords(trim($lastname.' '.$firstname.' '.$middlename));
		$chk = $this->conn->query("SELECT * FROM `drivers_list` where  license_id_no = '{$license_id_no}' ".($id>0? " and id!= '{$id}' " : ""))->num_rows;
		$this->capture_err();
		if($chk > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "Số giấy phép đã tồn tại trong hệ thống. Vui lòng kiểm tra lại.";
			return json_encode($resp);
			exit;
		}
		if(empty($id))
		...
		if($save){
			$resp['status'] = 'success';
			if(empty($id))
				$this->settings->set_flashdata('success',"Hồ sơ tài xế mới đã được lưu.");
			else
				$this->settings->set_flashdata('success',"Hồ sơ tài xế đã được cập nhật.");
	...
	function delete_driver(){
		...
		if($del){
			...
			if(is_file(base_app.$image_path))
				unlink((base_app.$image_path));
			$this->settings->set_flashdata('success',"Hồ sơ tài xế đã được xóa.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_offense_record(){
		extract($_POST);
		if(isset($_POST['evidence_path'])){
			$evidence_url = trim($_POST['evidence_path']);
			$_POST['evidence_path'] = $evidence_url;
			if($evidence_url == ''){
				$_POST['evidence_type'] = '';
			}else{
				$path = parse_url($evidence_url, PHP_URL_PATH);
				$ext = strtolower(pathinfo($path ?? '', PATHINFO_EXTENSION));
				$_POST['evidence_type'] = in_array($ext, array('mp4','webm','ogg','mov','m4v')) ? 'video' : 'image';
			}
		}
		$data = "";
		foreach($_POST as $k =>$v){
		...
		if($chk > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "Số biên bản đã tồn tại trong hệ thống. Vui lòng kiểm tra lại.";
			return json_encode($resp);
			exit;
		}
		...
		if(isset($_FILES['evidence']) && !empty($_FILES['evidence']['tmp_name'])){
			...
			if(!is_dir(base_app.$dir))
				mkdir(base_app.$dir, 0777, true);
			$ext = strtolower(pathinfo($_FILES['evidence']['name'], PATHINFO_EXTENSION));
			$fname = $dir.$driver_offense_id.'_'.time().'.'.$ext;
			$move = move_uploaded_file($_FILES['evidence']['tmp_name'], base_app.$fname);
			if($move){
				if(!empty($evidence_path) && is_file(base_app.$evidence_path))
					unlink(base_app.$evidence_path);
				$type = in_array($ext, array('mp4','webm','ogg')) ? 'video' : 'image';
				$this->conn->query("UPDATE `offense_list` set `evidence_path` = '{$fname}', `evidence_type` = '{$type}' where id = '{$driver_offense_id}' ");
				$this->capture_err();
			}
		}
		...
		if($save && $save2){
			if(empty($id))
				$this->settings->set_flashdata('success',"Biên bản vi phạm mới đã được lưu.");
			else
				$this->settings->set_flashdata('success',"Biên bản vi phạm đã được cập nhật.");
			$resp['status'] = 'success';
	...
	function delete_offense_record(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"Biên bản vi phạm đã được xóa.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
